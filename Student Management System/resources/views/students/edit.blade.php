        ...
        <form action="/student/{{ $student->id }}" method="POST" class="flex flex-col" enctype="multipart/form-data">
           @method('PUT')
            ...
            <div class="flex justify-center items-center my-4">
                @php 
                $default_profile = "https://api.dicebear.com/9.x/initials/svg?seed=".$student->first_name."-".$student->last_name.".svg"  
                @endphp
                <img class="w-[200px] h-[200px] object-cover rounded-full" src="{{$student->student_image ? asset("storage/student/".$student->student_image) : $default_profile}}" alt="">
            </div>
            ...
            <div class="mb-6 pt-3 rounded bg-gray-200">
                <label for="student_image" class="block text-gray-700 text-sm font-bold mb-2 ml-3">Student Profile</label>
            ...
            <button type="submit" class="bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 rounded shadow-lg hover:xl transition duration-200 mb-2">Update</button>

        </form>
        <form action="/student/{{ $student->id }}" method="POST">
            @method('delete')
            @csrf
            <button type="submit" class="w-full bg-red-600 hover:bg-purple-700 text-white font-bold py-2 rounded shadow-lg hover:xl transition duration-200 mb-2">Delete</button>
        </form>
            <button class="w-full bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 rounded shadow-lg hover:xl transition duration-200"><a href="/" class="block py-2 pr-4 pl-3">Back to student List</a></button>
    </section>
</main>
